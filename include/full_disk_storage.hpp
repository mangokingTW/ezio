...
struct full_disk_storage : storage_interface {
	full_disk_storage(file_storage const& fs, const std::string tp) : m_files(fs), target_partition(tp) {}
	...
	std::string find_partition(int index);
